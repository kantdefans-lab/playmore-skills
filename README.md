...
- 研究者画像：每次搜索后自动更新 `profile.md`，记录你的关键词权重、阅读偏好、概念图谱，越用越懂你
...
**为什么是 Qclaw，不是什么大家在说的 Claude 、ChatGPT或openclaw？**
...
在桌面手动新建一个文件夹，起名叫 `playmore`（右键桌面 → 新建文件夹）。

这就是你所有笔记的家。之后第一次运行 `/playmore` 时，它会自动发现这个文件夹。
...
第一次运行时，playmore 会问你在研究什么。随便跟它说，不用格式，不用关键词，就像跟人聊天一样：

```
我是社会学方向的博士生，主要研究数字经济对低收入群体劳动力市场的影响，
最近在看平台经济、零工经济这块，也关注 AI 对就业结构的冲击
```

它会从你说的话里提取关键词，生成你的研究者画像，存在 `playmore/profile.md` 里。之后每次搜索结束，这个文件都会自动更新——它会越来越懂你在研究什么，搜出来的东西也会越来越准。
...
**Step 4** — Create a folder on your Desktop named `playmore`. That's where all your notes will live.

**Step 5** — Ask Qclaw to create a one-click Chrome launcher on your Desktop:

```
Please create a desktop shortcut that launches Chrome in debug mode on port 9222
```

It will generate a `.bat` (Windows) or `.command` (macOS) file. Double-click it before each session.

**Step 6** — Run `/playmore` for the first time. It will ask about your research — just describe it naturally, no keywords needed:

```
I'm a sociology PhD student focusing on digital economy and labor market inequality,
especially platform work and AI's impact on employment structures.
```

playmore extracts keywords, builds your `profile.md`, and gets smarter with every search.
